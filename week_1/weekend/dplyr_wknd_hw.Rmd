...
```{r}
goodreads <- read_csv("/Users/user/Documents/CodeClan/codeclan_homework_AlisaTunonen/week_1/weekend/data/books.csv")
```
...
```{r}
goodreads %>%
  select(average_rating, publisher, title) %>%
...
```{r}
goodreads %>%
  select (average_rating, num_pages, title) %>%
  filter(num_pages < 50 & average_rating > 4) %>%
  arrange(desc(average_rating))
```
...
```{r}
goodreads %>%
  select(average_rating, language_code) %>%
  group_by(language_code) %>%
  summarise(mean = mean(average_rating, n = n()))
